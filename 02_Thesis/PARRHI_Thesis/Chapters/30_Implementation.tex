...
The \code{World} class has attributes that represent the state of the real world and discloses them to the PARRHI library. The \code{SetUIText} delegate similarly to the \code{MoveDelta} method from the \code{Robot} class, is a function pointer that can be configured from the library's host. This was done for the same reasons as with the \code{Robot} class - to create independence from the host.
...
The \code{Point} class, on the other hand, is a bit more interesting. The abstract \code{Point} class contains the three-dimensional position vector and some read-only properties, but the inheriting classes define how the position is set. For example the \code{PointRobot} class takes two joint indexes for its definition as it was described in section~\ref{Section:Points}. In each cycle, the public method \code{UpdatePoint} is invoked, which allows the \code{PointRobot} to calculate its new position. The \code{PointCamera} and \code{PointFix} do not add or overwrite any methods, since the \code{Point} class contains all needed functionalities. For clarity, some less important attributes and methods were omitted in these class diagrams.
...
The implementation of the PARRHI Core Routine is also straight forward. The container object first updates all \code{PointRobot} objects. It then loops through all its \code{Trigger} objects and checks their boolean expressions. If the trigger's \code{canTrigger} value and the mentioned expression both equal \code{true}, then the Core Routine invokes all actions from the trigger in question. If an action targets another object from the Parametrised Program (such as \code{SetTriggerStateActions} do with triggers) the action can directly be performed. \code{MoveRobotActions}, on the other hand, have to be sent to the Robot Library which will be explained in section~\ref{Section:RobotLibrary}.
