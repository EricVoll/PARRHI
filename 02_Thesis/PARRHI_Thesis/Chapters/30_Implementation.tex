...
Resetting the register's state at the very beginning (\code{Init()}) is crucial, since the data registers are persistent stores, which means, that their value is saved even when the controller is shut down. The Initialisation defines a clear behaviour at each launch of the program.
